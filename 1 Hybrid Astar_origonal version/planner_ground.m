function [partial_path,new_start,nodecount] = planner_ground(heightmap,pose_start,pose_goal)
% yaw: global frame
global trapcount
trapcount=0;
...
while ~isempty(open_list) && ~flag_takeoff
    ...
    [node_successor]=successor_gen_2D(node_current,pose_goal,heightmap,gx,gy); % generate successor nodes
...
function [node_successor]=successor_gen_2D(node_current,pose_goal, heightmap,gx,gy)
...
for i=imin:imax
    for j=jmin:jmax
        ...
        x_next=x_curr+i; next_y=y_curr+j; next_z=heightmap(x_next,next_y);
        next_yaw=neighbor2yaw(i,j);

        accessible=accessibility_check([x_curr, y_curr, yaw_curr],[x_next, next_y, next_yaw],gx,gy);
        if accessible
            node_successor(neighbor_count,1:4)=[x_next, next_y, next_z, next_yaw];
...
x_curr=curr_pos(1); y_curr=curr_pos(2); yaw_curr=curr_pos(3);
x_next=next_pos(1); y_next=next_pos(2); yaw_next=next_pos(3);
...
% -45deg: (gx_next-gy_next)/sqrt(2)
    ...
    switch yaw_next
        case 90
            ...
            standable=gx_next<max_roll&&gy_next<max_pitch;
        case -90
            traversable=var_gy<max_climb&&var_gy>max_descend;
            standable=gx_next<max_roll&&gy_next<max_pitch;
        case 0
            traversable=var_gx<max_climb&&var_gx>max_descend;
            standable=gx_next<max_pitch&&gy_next<max_roll;
        case 180
            traversable=var_gx<max_climb&&var_gx>max_descend;
            standable=gx_next<max_pitch&&gy_next<max_roll;
        case 45
            traversable=(var_gx+var_gy)/sqrt(2)<max_climb&&(var_gx+var_gy)/sqrt(2)>max_descend;
            standable=(gx_next+gy_next)/sqrt(2)<max_pitch&&(gx_next-gy_next)/sqrt(2)<max_roll;
        case -135
            traversable=(var_gx+var_gy)/sqrt(2)<max_climb&&(var_gx+var_gy)/sqrt(2)>max_descend;
            standable=(gx_next+gy_next)/sqrt(2)<max_pitch&&(gx_next-gy_next)/sqrt(2)<max_roll;
        case 135
            traversable=(var_gx-var_gy)/sqrt(2)<max_climb&&(var_gx-var_gy)/sqrt(2)>max_descend;
            standable=(gx_next+gy_next)/sqrt(2)<max_roll&&(gx_next-gy_next)/sqrt(2)<max_pitch;
        case -45
            traversable=(var_gx-var_gy)/sqrt(2)<max_climb&&(var_gx-var_gy)/sqrt(2)>max_descend;
            standable=(gx_next+gy_next)/sqrt(2)<max_roll&&(gx_next-gy_next)/sqrt(2)<max_pitch;
    end
    accessible=traversable&&standable;
%     fprintf('tr: %d;st: %d\n',traversable,standable);
%     if (~traversable)&&(~standable)
%         plot(y_next,x_next,'.','Color',[0.9 0 0]);
%         pause(0.01)
%     elseif ~traversable
%         plot(y_next,x_next,'.','Color',[0.5 0 0.9]);
%         pause(0.01)
%     elseif ~standable
%         plot(y_next,x_next,'.','Color',[0.9 0.5 0]);
%         pause(0.01)
%     end

...
%%
function [flag_landing,new_start_landing]=landing_judge(heightmap,gx,gy,node_current)
flying distance =sqrt((x_new_start-x_current)^2+(y_new_start-y_current)^2;
heightmap=heightmap(127:2595,107:4668)*0.05;
total_map_distance=sqrt((2595-127)^2+(4688-107)^2;
judgment_distance=total_map_distance*0.3;
if flying_distance>=judgment_distance
    if node_current(gx)<0.2&&node_current(gx)>-0.2&&node_current(gy)<0.2&&node_current(gy)>-0.2
        node_current(1:3)=heightmap(1:3);
        new_start_landing(1:3)=node_current(1:3);
        flag_landing=true;
    else
        flag_landing=false;
    end
